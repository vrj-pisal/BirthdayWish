async function logVisitor(pageName) {
  ...
  try {
    ...
    console.log('📤 Sending data to SheetDB:', payload);

    const sheetResponse = await fetch("https://sheetdb.io/api/v1/xynpa6kuoq8ag", {
      method: "POST",
      headers: {
    ...
    if (!sheetResponse.ok) {
      ...
      console.error("❌ SheetDB error:", sheetResponse.status, sheetResponse.statusText, errorText);
    } else {
      const result = await sheetResponse.json();
      console.log("✅ Data successfully logged to SheetDB:", result);
    }
